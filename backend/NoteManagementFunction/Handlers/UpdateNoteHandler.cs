...
namespace NeuroBrain.NoteManagementFunction.Handlers
{
    public class UpdateNoteHandler
    {
        ...
        public UpdateNoteHandler(NoteService noteService)
        {
        ...
        public async Task<APIGatewayHttpApiV2ProxyResponse> HandleAsync(APIGatewayHttpApiV2ProxyRequest request, string userId)
        {
            ...
            var updateRequest = JsonConvert.DeserializeObject<UpdateNoteRequest>(request.Body);
            updateRequest.NoteId = noteId;

            var note = await _noteService.UpdateNoteAsync(updateRequest, userId);
